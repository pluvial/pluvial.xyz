---
title: Tools
---

# Tools

[[cli-terminal-utilities|CLI and Terminal Utilities]]

[[productivity|Productivity]]

[[repls-editors|REPLs and Editors]]

[[text-editors|Text Editors]]

[[image-photo-editors|Image and Photo Editors]]

[[browsers|Browsers]]

[[build-systems|Build Systems]]

[[self-hosted|Self-Hosted]]

[[linux|Linux]]

[a11y.css](http://ffoodd.github.io/a11y.css/)

[Whitesource Renovate - Automated Dependency Updates](https://renovateapp.com/)

[webhint, the hinting engine for web best practices](https://webhint.io/)

[Flowchart Maker & Online Diagram Software](https://www.draw.io/)

[cubic-bezier.com](https://cubic-bezier.com/#.17,.67,.83,.67)

[Sportsurge](https://sportsurge.net/)

[Refactoring Databases -](https://databaserefactoring.com/index.html)

[Dependabot](https://dependabot.com/)

[Mockaroo - Random Data Generator and API Mocking Tool | JSON / CSV / SQL / Excel](https://mockaroo.com/)

[Database test data generator - Fill your database with random test data!](http://www.databasetestdata.com/)

[CNCF Cloud Native Interactive Landscape](https://landscape.cncf.io/)

[Robert Penner's Easing Functions](http://robertpenner.com/easing/)

[Easing Functions Cheat Sheet](https://easings.net/en)

[Write With Transformer](https://transformer.huggingface.co/)

[Hyper Island Toolbox](https://toolbox.hyperisland.com/)

[showthedocs](http://showthedocs.com/)

[What Web Can Do Today](https://whatwebcando.today/)

[The C4 model for visualising software architecture](https://c4model.com/)

[HTTP Archive](https://httparchive.org/)

[The Unix Game](https://www.unixgame.io/unix50)

[Clippy - CSS clip-path maker](https://bennettfeely.com/clippy/)

[Boxy SVG Editor](https://boxy-svg.com/)

[PrivacyTools - Encryption Against Global Mass Surveillance](https://www.privacytools.io/)

[https://insomnia.rest/](https://insomnia.rest/)

[GitHub - Kong/insomnia: The open-source, cross-platform API client for GraphQL, REST, and gRPC.](https://github.com/Kong/insomnia)

[Hoppscotch - Open source API development ecosystem](https://hoppscotch.io/)

[Hoppscotch docs](https://docs.hoppscotch.io/)

[HTTPie - command-line HTTP and API testing client for the API era](https://httpie.io/cli)

[GitHub - httpie/httpie: As easy as /aitch-tee-tee-pie/ 🥧 Modern, user-friendly command-line HTTP client for the API era. JSON support, colors, sessions, downloads, plugins & more. https://twitter.com/httpie](https://github.com/httpie/httpie)

[Curlie](https://curlie.io/)

[GitHub - rs/curlie: The power of curl, the ease of use of httpie.](https://github.com/rs/curlie)

[curl](https://curl.se/)

[GitHub - curl/curl: A command line tool and library for transferring data with URL syntax, supporting DICT, FILE, FTP, FTPS, GOPHER, GOPHERS, HTTP, HTTPS, IMAP, IMAPS, LDAP, LDAPS, MQTT, POP3, POP3S, RTMP, RTMPS, RTSP, SCP, SFTP, SMB, SMBS, SMTP, SMTPS, TELNET and TFTP. libcurl offers a myriad of powerful features](https://github.com/curl/curl)

[ImageOptim - better Save for Web](https://imageoptim.com/mac)

[Underdash](https://surma.github.io/underdash/)

[SVGOMG - SVGO's Missing GUI](https://jakearchibald.github.io/svgomg/)

[The Book of Shaders Editor](http://editor.thebookofshaders.com/)

[thiscodeWorks - Saving, organizing & sharing the best of code online](https://www.thiscodeworks.com/)

[Pika - Search npm for fast, modern packages.](https://www.pika.dev/npm/async/repl)

[Create App](https://createapp.dev/)

[AST explorer](https://astexplorer.net/)

[Compiler Explorer](https://godbolt.org/)

[Compiler Explorer](https://rust.godbolt.org/)

[[Wandbox]三へ( へ՞ਊ ՞)へ ﾊｯﾊｯ](https://wandbox.org/)

[mermaid - Markdownish syntax for generating flowcharts, sequence diagrams, class diagrams, gantt charts and git graphs.](https://mermaid-js.github.io/mermaid/#/)

[Diagram Software and Flowchart Maker](https://www.diagrams.net/)

[Manage HTML DOM with vanilla JavaScript](https://htmldom.dev/)

[The Binder Project](https://mybinder.org/)

[Functional Automated End-to-End Testing with Testim](https://go.testim.io/functional-automated-end-to-end-testing-with-testim-a)

[httpbin.org](https://httpbin.org/)

[What's your favorite JavaScript single LOC (line of code)](https://1loc.dev/)

[Microjs: Fantastic Micro-Frameworks and Micro-Libraries for Fun and Profit!](http://microjs.com/#)

[Openbase: JavaScript package insights and comparison](https://openbase.io/)

[Drop-in Minimal CSS](https://dohliam.github.io/dropin-minimal-css/)

[gifski - highest-quality GIF converter](https://gif.ski/)

[ImageOptim/gifski](https://github.com/ImageOptim/gifski)

[pngquant](https://pngquant.org/)

[Can I use... Support tables for HTML5, CSS3, etc](https://caniuse.com/)

[Ventoy](https://www.ventoy.net/en/index.html)

[Art of Chording](https://www.artofchording.com/)

[Home](http://www.openstenoproject.org/)

[http://www.openstenoproject.org/plover/](http://www.openstenoproject.org/plover/)

[Qwerty Steno - Home](http://qwertysteno.com/Home/)

[Apps](https://arieluna.notion.site/Apps-29ffc475bd3a4539b48eeb5bb551d713)

[ianyh/Amethyst: Automatic tiling window manager for macOS à la xmonad.](https://github.com/ianyh/Amethyst)

[koekeishiya/yabai: A tiling window manager for macOS based on binary space partitioning](https://github.com/koekeishiya/yabai)

[koekeishiya/skhd: Simple hotkey daemon for macOS](https://github.com/koekeishiya/skhd)

[kasper/phoenix: A lightweight macOS window and app manager scriptable with JavaScript](https://github.com/kasper/phoenix)

[All* my macOS keyboard shortcuts - jamieonkeys](https://www.jamieonkeys.dev/posts/keyboard-shortcuts/)

[Pikchr: Documentation](https://pikchr.org/home/doc/trunk/homepage.md)

[Asymptote](https://asymptote.sourceforge.io/)

[opensumi/core: 🚀 A framework helps you quickly build Cloud or Desktop IDE products.](https://github.com/opensumi/core)

[Pika – Create beautiful mockups from screenshots](https://pika.style/)

[[Array Builder]](https://www.arraybuilder.com/)

[dexterleng/vimac: Productive macOS keyboard-driven navigation](https://github.com/dexterleng/vimac)

[massCodeIO/massCode: A free and open source code snippets manager for developers](https://github.com/massCodeIO/massCode)

[massCode | A free and open source code snippets manager for developers](https://masscode.io/)

[Volumes · Fly Docs](https://fly.io/docs/reference/volumes/)

[fly.io](https://github.com/superfly/)

[Railway](https://railway.app/)

[Ilograph Interactive System Diagrams](https://www.ilograph.com/)

[https://code.sgo.to/typograms/](https://code.sgo.to/typograms/)

[winsiderss/systeminformer: A free, powerful, multi-purpose tool that helps you monitor system resources, debug software and detect malware. Brought to you by Winsider Seminars & Solutions, Inc. @ http://www.windows-internals.com](https://github.com/winsiderss/systeminformer)

[NotABug.org: Free code hosting](https://notabug.org/)

[theonedev/onedev: Self-hosted Git Server with CI/CD and Kanban](https://github.com/theonedev/onedev)

[https://code.onedev.io/projects/160/files](https://code.onedev.io/projects/160/files)

[Phind: AI search engine](https://www.phind.com/)

[blosxom :: the zen of blogging ::](http://www.blosxom.com/)

[Annotated blosxom.cgi](https://web.archive.org/web/20100712013556/http://www.hecker.org/blosxom/blosxom2-footnotes-folded)

[https://tinyapps.org/blog/](https://tinyapps.org/blog/)

[Rob Reed builds a better Blosxom with Ode](https://tinyapps.org/blog/201003250700_rob_reed_ode.html)

[Bear Blog](https://bearblog.dev/)

[npat-efault/picocom: Minimal dumb-terminal emulation program](https://github.com/npat-efault/picocom)

[Nimbus - One Click Dev Environments](https://www.usenimbus.com/)

[Ephemeral Environments | Coherence](https://www.withcoherence.com/)

[ravynOS – Finesse of macOS. Freedom of FreeBSD.](https://ravynos.com/)

[Untitled - Cruncher](https://cruncher.io/?/-NVjM7_0TRZ5lb8spXqX)

[osnr/cruncher: Scrubbing calculator.](https://github.com/osnr/cruncher)

[NumPad](https://numpad.io/)

[Soulver 3 - Notepad Calculator App for Mac](https://soulver.app/)

[Numbr](https://numbr.dev/)

[filipesabella/CalcPad: A different take on the caculator](https://github.com/filipesabella/calcpad)

[facebook/infer: A static analyzer for Java, C, C++, and Objective-C](https://github.com/facebook/infer)

[Infer Static Analyzer | Infer | Infer](https://fbinfer.com/)

[(298) William Byrd on "The Most Beautiful Program Ever Written" [PWL NYC] - YouTube](https://www.youtube.com/watch?v=OyfBQmvr2Hc)

[(315) "Finding bugs without running or even looking at code" by Jay Parlar - YouTube](https://www.youtube.com/watch?v=FvNRlE4E9QQ)

[kgrgreer/foam3: FOAM: Feature-Oriented Active Modeller, Version 3 (unstable)](https://github.com/kgrgreer/foam3#videos)

[iOS Privacy: Announcing InAppBrowser.com - see what JavaScript commands get injected through an in-app browser · Felix Krause](https://krausefx.com/blog/announcing-inappbrowsercom-see-what-javascript-commands-get-executed-in-an-in-app-browser)

[Introducing Jel: the video game for work | by Greg Fodor | Medium](https://gfodor.medium.com/introducing-jel-the-un-zoom-320d3dcfd8f6)

[Linux perf Examples](https://brendangregg.com/perf.html)

[EvanLi/Github-Ranking: Github Ranking Github stars and forks ranking list. Github Top100 stars list of different languages. Automatically update daily. | Github仓库排名，每日自动更新](https://github.com/EvanLi/Github-Ranking)

[IndexStorm/git-rec-ext: Frontend code for GitHub Recommendation Extension](https://github.com/IndexStorm/git-rec-ext)

[OSS Insight](https://ossinsight.io/#trending-repos)

[Build a Better GitHub Insight Tool in a Week? A True Story | OSS Insight](https://ossinsight.io/blog/why-we-choose-tidb-to-support-ossinsight/)

[LibHunt - Trending open-source projects and their alternatives](https://www.libhunt.com/)

[ekzhang/classes.wtf: A course catalog with extremely fast full-text search](https://github.com/ekzhang/classes.wtf)

[Syncthing](https://syncthing.net/)

[Maestral](https://maestral.app/)

[Zstandard - Real-time data compression algorithm](http://facebook.github.io/zstd/)

[gchq/CyberChef: The Cyber Swiss Army Knife - a web app for encryption, encoding, compression and data analysis](https://github.com/gchq/CyberChef)

[CyberChef](https://gchq.github.io/CyberChef/)

[Linear – A better way to build products](https://linear.app/)

[Use plaintext email](https://useplaintext.email/)

[Binder Documentation — Binder 0.1b documentation](https://mybinder.readthedocs.io/en/latest/)

[jupyterhub/binderhub: Run your code in the cloud, with technology so advanced, it feels like magic!](https://github.com/jupyterhub/binderhub)

[Binder](https://mybinder.org/)

[Reddit's Favorite Products in One Place](https://www.looria.com/reddit)

[WOWCUBE® ENTERTAINMENT SYSTEM](https://wowcube.com/?buypageredirect)

[hakimel/reveal.js: The HTML Presentation Framework](https://github.com/hakimel/reveal.js)

[The HTML presentation framework | reveal.js](https://revealjs.com/)

[Slides – Create and Share Presentations for Free](https://slides.com/)

[slidevjs/slidev: Presentation Slides for Developers](https://github.com/slidevjs/slidev)

[Home | Slidev](https://sli.dev/#overview)

[Taildrop · Tailscale](https://tailscale.com/kb/1106/taildrop/)

[Tailscale · Best VPN Service for Secure Networks](https://tailscale.com/)

[Koch method to learn Morse](https://stendec.io/morse/koch.html)

[solitude - Careful Words](https://carefulwords.com/solitude)

[ahmed-debbech/mutak: Mutak : a Music Tracker for Spotify](https://github.com/ahmed-debbech/mutak)

[TigYog](https://tigyog.app/)

[SILE – Home](https://sile-typesetter.org/)

[gfngfn/SATySFi: A statically-typed, functional typesetting system](https://github.com/gfngfn/SATySFi)

[Lout (software) - Wikipedia](https://en.wikipedia.org/wiki/Lout_(software)?useskin=vector)

[Scribe (markup language) - Wikipedia](https://en.wikipedia.org/wiki/Scribe_(markup_language)?useskin=vector)

[apenwarr/blip: A tool for seeing your Internet latency. Try it at http://gfblip.appspot.com/](https://github.com/apenwarr/blip)

[Pinry: A Selfhosted Alternative to Pinterest | Hacker News](https://news.ycombinator.com/item?id=30981280)

[pinry/pinry: Pinry, a tiling image board system for people who want to save, tag, and share images, videos and webpages in an easy to skim through format. It's open-source and self-hosted.](https://github.com/pinry/pinry)

[Pinry Docs](https://docs.getpinry.com/)

[Raindrop.io — All-in-one bookmark manager](https://raindrop.io/)

[Eagle - Organize all your reference images in one place](https://en.eagle.cool/)

[rustdesk/rustdesk: Open source virtual / remote desktop infrastructure for everyone! The open source TeamViewer alternative. Display and control your PC and Android devices from anywhere at anytime.](https://github.com/rustdesk/rustdesk)

[Womp: The New Way to 3D](https://www.womp.com/)

[Runnaroo | A Better Private Search Engine](https://www.runnaroo.com/)

[Kagi Search](https://kagi.com/)

[Neeva - Ad-free, private search](https://neeva.com/)

[Flowshot: AI For Google Sheets | AI Formula Plugin](https://flowshot.ai/)

[ReVanced](https://github.com/revanced)

[yuliskov/SmartTubeNext: Ad free app for watching tube videos on Android TV boxes](https://github.com/yuliskov/SmartTubeNext#Installation)

[publint](https://publint.dev/)

[pluvial - Codeberg.org](https://codeberg.org/pluvial)

[Improving Codeberg | Codeberg Documentation](https://docs.codeberg.org/improving-codeberg/#donate-to-codeberg)

[Retheme.org - your window to a better internet](https://retheme.org/)

[Free Photo editor online image animation and design : Pixlr](https://pixlr.com/)

[Jitsi Meet](https://meet.jit.si/)

[Free Video Conferencing Software for Web & Mobile | Jitsi](https://jitsi.org/)

[Raycast](https://www.raycast.com/)

[The Browser Company | Building Arc](https://thebrowser.company/)

[Arc from The Browser Company](https://arc.net/)

[yk Adas · Designer · Available for freelance · Digital products · Blockchain · Speculative · Research Patr](http://patrykadas.com/browser.html)

[Open Source Insights](https://deps.dev/)

[Monodraw for macOS — Helftone](https://monodraw.helftone.com/)

[Your apps. Deployed on AWS. In minutes. With Microtica.](https://www.microtica.com/)

[Brokkr | Hydra Host](https://hydrahost.com/brokkr/)

[Flightcontrol — AWS Without Pain](https://www.flightcontrol.dev/)

[SST](https://sst.dev/)

[Cloudmos](https://cloudmos.io/)

[Markflow - Supercharge your workflow](https://markflow.app/)

[Open Source Alternative To | Hacker News](https://news.ycombinator.com/item?id=31589924)

[Open Source Alternatives to Proprietary Software](https://www.opensourcealternative.to/)

[Docker-based local development environments in minutes. | DDEV](https://ddev.com/)

[Notaku - Create your company essential websites with Notion](https://notaku.so/)

[Inspect Source Code Performance with Deoptigate](https://www.builder.io/blog/deoptigate)

[Understanding Monomorphism to Improve Your JS Performance up to 60x](https://www.builder.io/blog/monomorphic-javascript)

[thlorenz/deoptigate: ⏱️ Investigates v8/Node.js function deoptimizations.](https://github.com/thlorenz/deoptigate)

[Why the New V8 is so Damn Fast - NodeSource](https://nodesource.com/blog/why-the-new-v8-is-so-damn-fast)

[Tracking Down and Fixing Performance Bottlenecks with Deoptigate](https://nodesource.com/blog/tracking-down-performance-bottlenecks-nsolid-deoptigate)

[Glottolog 4.7 -](https://glottolog.org/)

[evilsocket/arc: A manager for your secrets.](https://github.com/evilsocket/arc)

[Authy | Two-factor Authentication (2FA) App & Guides](https://authy.com/)

[Password & Secrets Management | Keeper Security](https://www.keepersecurity.com/)

[arabisaldrin/quick-pick](https://github.com/arabisaldrin/quick-pick)

[n8n.io - a powerful workflow automation tool](https://n8n.io/)

[Nimble - Your Effortless Web Data Gathering Solution](https://www.nimbleway.com/)

[Hop](https://hop.io/)

[Platformatic](https://platformatic.dev/)

[www.marginalia.nu @ marginalia.nu](https://www.marginalia.nu/)

[Marginalia Search](https://search.marginalia.nu/)

[Wexond](https://github.com/wexond)

[Hammerspoon](http://www.hammerspoon.org/)

[Hammerspoon/hammerspoon: Staggeringly powerful macOS desktop automation with Lua](https://github.com/Hammerspoon/hammerspoon)

[glyph/Pomodouroboros: Pomodoro timer that acknowledges the inexorable, infinite passage of time](https://github.com/glyph/Pomodouroboros)

[Presentation software for fast-moving teams | Pitch](https://pitch.com/)
