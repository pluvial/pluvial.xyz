# XML

[XML.com](https://www.xml.com/)

[Extensible Markup Language (XML)](https://www.w3.org/XML/)

[XML Technology](https://www.w3.org/standards/xml/)

[The Extensible Stylesheet Language Family (XSL)](https://www.w3.org/Style/XSL/)

[W3C XML Query (XQuery)](https://www.w3.org/XML/Query/)

[Efficient Extensible Interchange Working Group](https://www.w3.org/XML/EXI/)

[XML: Extensible Markup Language | MDN](https://developer.mozilla.org/en-US/docs/Web/XML)

[XPath | MDN](https://developer.mozilla.org/en-US/docs/Web/XPath)

[XSLT: Extensible Stylesheet Language Transformations | MDN](https://developer.mozilla.org/en-US/docs/Web/XSLT)

[RELAX NG home page](https://relaxng.org/)

[schematron.com](https://schematron.com/)

[schxslt/schxslt](https://github.com/schxslt/schxslt)